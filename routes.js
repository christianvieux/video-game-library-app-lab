...
router.get("/games/platform/:platform/genre/:genre", async (req, res) => {
  ...
  const games = await Game_Library.find(query);
  res.render(
    "games/platform/genre/index.ejs",
    { platform, games, genre}
  );
});

// POST a new game
router.post("/games", async (req, res) => {
  const {title, genre, platform, rating, cover_url} = Object.fromEntries(
    Object.entries(req.body).map(([key, value]) => [key.toLowerCase(), value.toLowerCase()])
  );
...
router.put('/games/:id', async (req, res) => {
  try {
      await Game_Library.findByIdAndUpdate(req.params.id, Object.fromEntries(
        Object.entries(req.body).map(([key, value]) => [key.toLowerCase(), value.toLowerCase()])
      ));
